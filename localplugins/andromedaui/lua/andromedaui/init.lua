---AndromedaVim UI Utilities
---
---UI utility functions to use within AndromedaVim and user configurations.
---
---This module can be loaded with `local andromeda = require "andromedaui"`
---
---copyright 2023
...
M.config = require "andromedaui.config"

---@param opts AndromedaUIOpts
function M.setup(opts)
  M.config = vim.tbl_deep_extend("force", M.config, opts)

  vim.api.nvim_create_autocmd("ColorScheme", {
    desc = "Load custom highlights from user configuration",
    group = vim.api.nvim_create_augroup("andromedavim_highlights", { clear = true }),
    callback = function()
      ...
      vim.api.nvim_exec_autocmds("User", { pattern = "AndromedaColorScheme", modeline = false })
    end,
  })
...
--- Get an icon from the AndromedaVim internal icons if it is available and return it
---@param kind string The kind of icon in andromedaui.icons to retrieve
...
--- Get a icon spinner table if it is available in the AndromedaVim icons. Icons in format `kind1`,`kind2`, `kind3`, ...
