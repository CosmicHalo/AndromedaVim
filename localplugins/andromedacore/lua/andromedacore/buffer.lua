---AndromedaVim Buffer Utilities
---
---Buffer management related utility functions
---
---This module can be loaded with `local buffer_utils = require "andromedacore.buffer"`
---
---copyright 2023
---license GNU General Public License v3.0
---@class andromedacore.buffer
local M = {}

local andromeda = require "andromedacore"

M.sessions = andromeda.config.sessions
...
function M.nav_to(tabnr)
  if tabnr > #vim.t.bufs or tabnr < 1 then
    andromeda.notify(("No tab #%d"):format(tabnr), vim.log.levels.WARN)
  else
...
function M.prev()
  if vim.fn.bufnr() == M.current_buf then
    if M.last_buf then
      ...
      andromeda.notify("No previous buffer found", vim.log.levels.WARN)
    end
  else
    andromeda.notify("Must be in a main editor window to switch the window buffer", vim.log.levels.ERROR)
  end
end
...
function M.close(bufnr, force)
  ...
  if andromeda.is_available "mini.bufremove" and M.is_valid(bufnr) and #vim.t.bufs > 1 then
...
---@param compare_func string|function a string of a comparator defined in require("andromedacore.buffer.comparator") or a custom comparator function
---@param skip_autocmd boolean|nil whether or not to skip triggering AndromedaBufsUpdated autocmd event
---@return boolean # Whether or not the buffers were sorted
function M.sort(compare_func, skip_autocmd)
  if type(compare_func) == "string" then compare_func = require("andromedacore.buffer.comparator")[compare_func] end
  if type(compare_func) == "function" then
    ...
    if not skip_autocmd then andromeda.event "BufsUpdated" end
