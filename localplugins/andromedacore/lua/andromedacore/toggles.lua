---AndromedaVim UI/UX Toggles
---
--- Utility functions for easy UI toggles.
---
---This module can be loaded with `local ui = require("andromedacore.toggles")`
---
---copyright 2023
---license GNU General Public License v3.0
---@class andromedacore.toggles
local M = {}

local function bool2str(bool) return bool and "on" or "off" end
local function ui_notify(silent, ...) return not silent and require("andromedacore").notify(...) end

--- Toggle notifications for UI toggles
---@param silent? boolean if true then don't sent a notification
function M.notifications(silent)
  local features = require("andromedacore").config.features
...
function M.autopairs(silent)
  ...
  if ok then
    ...
    require("andromedacore").config.features.autopairs = autopairs.state.disabled
...
function M.buffer_cmp(bufnr, silent)
  ...
  if vim.b[bufnr].cmp_enabled == nil then vim.b[bufnr].cmp_enabled = require("andromedacore").config.features.cmp end
...
function M.buffer_syntax(bufnr, silent)
  ...
  local andromedalsp_avail, lsp_toggle = pcall(require, "andromedalsp.toggles")

  if vim.bo[bufnr].syntax == "off" then
    if ts_avail and parsers.has_parser() then vim.treesitter.start(bufnr) end
    vim.bo[bufnr].syntax = "on"
    if andromedalsp_avail and not vim.b[bufnr].semantic_tokens then lsp_toggle.buffer_semantic_tokens(bufnr, true) end
  else
    if ts_avail and parsers.has_parser() then vim.treesitter.stop(bufnr) end
    vim.bo[bufnr].syntax = "off"
    if andromedalsp_avail and vim.b[bufnr].semantic_tokens then lsp_toggle.buffer_semantic_tokens(bufnr, true) end
  end
...
function M.url_match(silent)
  ...
  require("andromedacore").set_url_match()
