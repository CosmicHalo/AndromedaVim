---AndromedaVim Buffer Comparators
---
---Buffer comparator functions for sorting buffers
---
---This module can be loaded with `local buffer_comparators = require "andromedacore.buffer.comparator"`
---
---copyright 2023
---license GNU General Public License v3.0
---@class andromedacore.buffer.comparator
local M = {}

local fnamemodify = vim.fn.fnamemodify
local function bufinfo(bufnr) return vim.fn.getbufinfo(bufnr)[1] end
local function unique_path(bufnr)
  local status_avail, provider = pcall(require, "andromedaui.status.provider")
  if not status_avail then
    vim.notify("AndromedaUI required for unique path calculation", vim.log.levels.ERROR, { title = "AndromedaVim" })
    return ""
  end
